...
import {Effects, EffectsContext, EffectsHandlers, IField, Paths, SchemaFormField, SchemaFormStore} from '../../types';
...
const Form: any = Vue.extend({
  props: {
    ...
    mode: {type: String as PropType<'edit' | 'display' | undefined>},
    editable: {type: Boolean, default: true},
    effects: {type: Function as PropType<Effects>},
    schema: {type: Object as PropType<SchemaFormField>, required: true},
  ...
  provide() {
    ...
    return {
      [SCHEMA_FORM_STORE_INJECT_KEY]: Vue.observable(store) as SchemaFormStore
    };
  },
  data() {
    return {
  ...
  watch: {
    readonly: {
      immediate: true,
      handler(readonly: boolean) {
        this.store.readonly = readonly;
      }
    },
    disabled: {
      immediate: true,
      handler(disabled: boolean) {
  ...
  methods: {
    ...
    createContext(): EffectsContext {
      ...
      context.validate = async (handler) => {
        const errors = await runValidation(this.currentValue, store.fields, true);
        if (handler) {
    ...
    async onOk(forceValidate: boolean,
               callback?: (value) => any) {
    ...
    createSubmitButton(text: string = '', btnProps: object = null, action: () => any = null) {
      const hasOkHandler = hasListener(this, 'ok');
      if (!hasOkHandler) {
        return null;
      }
      const buttonProps = btnProps || (this.schema.props && this.schema.props.okProps) || {};
      if (!buttonProps.type) {
      ...
      return this.createButton(
          text || this.schema.props && this.schema.props.okText || '提交',
          action || (() => {
            this.onOk(true);
    ...
    createCancelButton(text: string = '', btnProps: object = null, action: () => any = null) {
    ...
    createResetButton(text: string = '', btnProps: object = null, action: () => any = null) {
  ...
  render() {
    const {title, sticky, prefixCls, currentValue, schema} = this;
    const store = this.store;
