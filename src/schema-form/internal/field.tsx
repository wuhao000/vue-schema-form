...
@Component({
  ...
  provide() {
    return {
      [SCHEMA_FIELD_INJECT_KEY]: this['field'],
      [SCHEMA_FIELD_FORM_VALUE_INJECT_KEY]: this['formValue']
    };
  }
})
export default class FormField extends mixins(Emitter) {
  ...
  public renderInputComponent() {
    ...
    if (inputFieldDef.layout) {
      ...
      const noWrap = isNull(this.field.title);
    ...
    if (this.definition.slots) {
      const slotsDef = this.definition.slots;
      Object.keys(slotsDef).forEach(slotName => {
        if (typeof slotsDef[slotName] === 'string') {
          if (this.store.root.$slots && this.store.root.$slots[slotsDef[slotName] as string]) {
            slots[slotName] = this.store.root.$slots[slotsDef[slotName] as string];
          }
        } else {
    ...
    if (this.definition.scopedSlots) {
      const slotsDef = this.definition.scopedSlots;
      Object.keys(slotsDef).forEach(slotName => {
        if (typeof slotsDef[slotName] === 'string') {
          if (this.store.root.$scopedSlots && this.store.root.$scopedSlots[slotsDef[slotName] as string]) {
            scopedSlots[slotName] = this.store.root.$scopedSlots[slotsDef[slotName] as string];
          }
        } else {
          scopedSlots[slotName] = slotsDef[slotName];
        }
      });
    }
    // @ts-ignore
  ...
  public getRules(type?: string) {
    ...
    if (type) {
      return rules.filter(it => it.trigger === type);
    }
    return rules;
  }
